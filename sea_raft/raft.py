"""SEA-RAFT optical flow network.

Math-identical to the official SEA-RAFT ``core/raft.py`` so the released
``Tartan-C-T-TSKH-spring540x960-M`` checkpoint loads exactly. Changes vs. upstream:

* ``PyTorchModelHubMixin`` (and the ``huggingface_hub`` dependency) is dropped --
  weights are loaded from the local ``.pth`` file shipped under ``pretrained_models/``.
* Imports are relative so ``sea_raft`` works as an installed sub-package.

In F2HDR this network is used **frozen** as a coarse optical-flow estimator; see
:class:`models.sea_raft_flow.FrozenSEARAFT`.
"""
import math
...
class RAFT(nn.Module):
    ...
    def initialize_flow(self, img):
        """Flow is the difference between two coordinate grids: flow = coords2 - coords1."""
        N, C, H, W = img.shape
    ...
    def forward(self, image1, image2, iters=None, flow_gt=None, test_mode=False):
        ...
        ``image1`` / ``image2`` are expected in the [0, 255] range (the network applies
        its own normalization ``2 * x / 255 - 1`` internally).
        """
        ...
        if not test_mode:
            ...
            return {"final": flow_predictions[-1], "flow": flow_predictions,
                    "info": info_predictions, "nf": nf_predictions}
        return {"final": flow_predictions[-1], "flow": flow_predictions,
                "info": info_predictions, "nf": None}
